...
public class InventoryGUI : GUIComponent
{
    ...
    {
        DisconnectFromCurrentInventory();
        targetInventory = inv;
        numFrames = inv.inventoryCapacity;
        SetScrollAmount(0);
        inv.myInventoryGUIs.Add(this);
        UpdateGUI();
    }

    public void DisconnectFromCurrentInventory()
    {
        if(targetInventory) targetInventory.myInventoryGUIs.Remove(this);
    }
    ...
    void UpdateFrameLocations()
    {
        ...
        for (int i = 0; i < pickupDropList.Count; i++)
        {
            ...
            pickupDropList[i].transform.position = transform.position + new Vector3(currentWidth, - currentHeight + (scrollAmount * (scale + separation.y)), -1);
            ...
            {
                //update the actualSize.x in case we do not wrap.
                float newActualWidth = currentWidth - (separation.x);
                if (newActualWidth > actualSize.x) actualSize.x = newActualWidth;
            }
    ...
    public override void Align()
    {
        ...
        myCollider.offset = new Vector2(xAlign + (myCollider.size.x / 2), -yAlign - (myCollider.size.y / 2));
    }
