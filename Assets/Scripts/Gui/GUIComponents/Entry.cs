...
public class Entry : GUIComponent
{
    ...
    float GetLineWidth(GameObject line)
    {
        ...
        {
            w -= childContainers[i].GetWidth() * transform.lossyScale.x;
            i--;
        }
    ...
    public override void Align()
    {
        ...
        for ( int i = 0; i < lines.Count; i++)
        {
            ...
            lines[i].transform.position = transform.position + new Vector3(xAlign, ((i+1) * heightAdjust * transform.lossyScale.y * -1) - yAlign, 0);
        }
        Vector2 myDimension = GetDimensions();

        myCollider.size = new Vector2(myDimension.x * (1/ transform.lossyScale.x), myDimension.y * (1 / transform.lossyScale.y));
        myCollider.offset = new Vector2((myCollider.size.x / 2), -(yAlign * (1 / transform.lossyScale.y)) - (myCollider.size.y / 2));
    }
