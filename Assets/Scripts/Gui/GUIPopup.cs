...
public abstract class GUIPopup : GUIComponent
{
    ...
    private void Start()
    {
        Assert.IsNotNull(popupControllerInstance);
    }
    ...
    private bool PositionWithinPopupBounds(Vector2 targetPosition)
    {
        ...
        if (targetPosition != null && size.x <= popupControllerInstance.popupBounds.x && size.y <= popupControllerInstance.popupBounds.y)
        {
            ...
            float maxVerticalPosition = popupControllerInstance.transform.position.y - popupControllerInstance.popupBounds.y + size.y;
            ...
            transform.position = new Vector3(Mathf.Clamp(targetPosition.x, popupControllerInstance.transform.position.x, maxHorizontalPosition), 
                                             Mathf.Clamp(targetPosition.y, popupControllerInstance.transform.position.y, maxVerticalPosition), 
